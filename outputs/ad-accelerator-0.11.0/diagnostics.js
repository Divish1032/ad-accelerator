...
document.getElementById('run').addEventListener('click',async()=>{
 ...
 try{
   ...
   for(const [tabId,entry] of Object.entries(state).filter(([,e])=>sessions.some(r=>r.id===e.id)).slice(0,5)){
     const foreign=await chrome.declarativeNetRequest.testMatchOutcome({url:'https://strict-check.invalid/test',type:'main_frame',tabId:Number(tabId),topUrl:'https://'+entry.host+'/'});
     lines.push((foreign.matchedRules.some(r=>r.ruleId===entry.id)?'PASS':'FAIL')+' strict external navigation · '+entry.host);
     const own=await chrome.declarativeNetRequest.testMatchOutcome({url:'https://'+entry.host+'/test',type:'main_frame',tabId:Number(tabId),topUrl:'https://'+entry.host+'/'});
