# Ad Accelerator 0.11.0 — Chrome release candidate
...
The new page explains that IP, approximate location and browser/account identity are not hidden. No VPN, proxy service, fingerprint spoofing or new telemetry is included. Privacy controls are separate from ad-protection pause; link cleanup follows network/site pauses. Details, porting boundaries, data handling and primary API references: [PRIVACY.md](PRIVACY.md).
...
- **Prime Video / JioHotstar / YouTube:** existing on-demand detection and configurable speed retained. Prefer skipping clicks an enabled, visible ad-specific Skip control. If an ad remains, acceleration is the fallback.
- **Hotstar separate ad media:** can try seeking near the end only when the ad-only video exposes a finite, seekable timeline. If it rejects or does not end the ad, fall back after one second. This new seeking behavior is experimental and not service-verified.
...
Prior user confirmation: Prime 10× and Hotstar 0.2.0 acceleration worked. Prior YouTube normal-video detection was verified on a Premium session. New skipping, generic adapters, website-wide blocking and live-service compatibility still need installed Chrome verification. In particular no claim is made that luciferdonghua.in or every embedded host is fully covered.
