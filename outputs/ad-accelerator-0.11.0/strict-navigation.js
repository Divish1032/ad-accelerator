(function(root){
 ...
 class StrictNavigation{
  ...
  async apply(tabId,host,c){
   ...
   await this.chrome.declarativeNetRequest.updateSessionRules({removeRuleIds:[entry.id,entry.id+1],addRules:host&&this.strict.active(c,host,this.core)?this.strict.rules(tabId,host,c.strict[host]||[],entry.id):[]});
  ...
  async committed(d,c){if(d.frameId!==0||d.url==='about:blank')return;const h=this.core.host(d.url);if(h)await this.apply(d.tabId,this.strict.active(c,h,this.core)?h:null,c);}
