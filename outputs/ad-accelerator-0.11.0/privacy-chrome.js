(function(root){
 ...
 class ChromePrivacy{
  ...
  async syncLinks(p,c){
   await this.api.declarativeNetRequest.updateDynamicRules({removeRuleIds:this.core.LINK_IDS,addRules:this.core.linkRules(p,c)});
  }
  ...
  async syncLocation(p,c={}){
   ...
   for(const o of p.locationOrigins.filter(o=>this.siteActive(c,o)))await this.api.contentSettings.location.set({primaryPattern:'<all_urls>',secondaryPattern:o+'/*',setting:'block',scope:'regular'});
  }
  ...
  async status(tabId,c={}){
   ...
   return {preferences:p,origin:o,siteActive,cookiesSuspended:this.cookiesSuspended(c),linksInstalled:linkRules.some(r=>r.id===20&&r.action.type==='redirect'),cookies,location:{available:locationAvailable,setting:l?.setting||'unknown',error:l?.error,owned:p.locationOrigins.includes(o),active:siteActive&&p.locationOrigins.includes(o)},cleanupAvailable:await this.permission('browsingData')&&!!this.api.browsingData?.remove};
  }
  async prepare(tabId){
