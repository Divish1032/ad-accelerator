(() => {
 ...
 const {siteFor,onDemandPath,inspectHotstar,inspectYouTube}=AdAcceleratorSites;
 const site=siteFor(location.hostname),accelerator=new Accelerator(),skipper=new VideoActions.Skipper();
 ...
 function disconnect(){
   ...
   config.enabled=false;adaptiveElements?.stop();picker.stop();custom.restore();compatibility.restore();accelerator.stop('Extension disconnected; refresh this tab');skipper.reset();css?.remove();
 ...
 function tick(){
   ...
   try{
     ...
     if(eligible){
       if(site==='prime'){result=inspectPrimePlayer(document,getComputedStyle,isAdLabel);player=document.querySelector('[aria-label="Web Player"]');}
       else if(site==='hotstar'){result=inspectHotstar(document,getComputedStyle);player=document.querySelector('[data-testid="player-space-container"]');}
       else if(site==='youtube'){result=inspectYouTube(document,getComputedStyle);player=document.querySelector('#movie_player');}
       else{result=VideoActions.generic(document,getComputedStyle);player=result.player;}
     }
 ...
 addEventListener('pagehide',()=>{stopped=true;adaptiveElements?.update([],false,false);picker.stop();accelerator.stop('Page closed');skipper.reset();});
