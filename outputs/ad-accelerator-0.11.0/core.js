(function (root) {
  ...
  class Accelerator {
    constructor() {
      ...
      this.candidate = null;
      this.blocked = false;
      this.adEvidence=null;this.blockedEvidence=null;this.rearm=null;this.retries=0;
    ...
    update({ video, ad, enabled, now, speed = 10, live = false, adInfo = null }) {
      ...
      if (!ad) {this.blocked=false;this.blockedEvidence=null;this.rearm=null;this.retries=0;}
      if(this.blocked&&ad&&enabled&&video&&!video.paused&&adInfo&&this.blockedEvidence&&this.retries<3){
        const old=this.blockedEvidence;
        const newOrdinal=adInfo.total===old.total&&adInfo.ordinal>old.ordinal;
        const newCountdown=adInfo.remaining>old.remaining+2;
        if(!this.rearm&&(newOrdinal||newCountdown))this.rearm={video,info:adInfo,time:video.currentTime,now};
        const r=this.rearm;
        if(r&&r.video===video&&adInfo.ordinal===r.info.ordinal&&adInfo.remaining<r.info.remaining&&video.currentTime>r.time&&now-r.now>=250){this.blocked=false;this.rearm=null;this.candidate=null;this.retries++;}
      }
