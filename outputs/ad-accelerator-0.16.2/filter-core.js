(function(root){
 ...
 function strictSites(raw){return Object.fromEntries(Object.entries(raw||{}).filter(([h,v])=>domain(h)&&Array.isArray(v)).slice(0,50).map(([h,v])=>[h,[...new Set(v.filter(d=>domain(d)))].slice(0,20)]));}
  ...
  function compatibility(){
    // Plex free VOD refuses to create its player when the Google IMA ad
    // stack is blocked. The earlier XHR-only allowance did not restore a fresh
    // protected player, so allow its subresources to the same two domains only
    // under the exact watch host. Top-level navigation remains blocked.
    return [{id:90,priority:20,action:{type:'allow'},condition:{
      requestDomains:['g.doubleclick.net','pagead2.googlesyndication.com'],
      topDomains:['watch.plex.tv'],resourceTypes:RESOURCE_TYPES.filter(type=>type!=='main_frame')
    }},{id:91,priority:20,action:{type:'allow'},condition:{
      // WeTV also refuses to create its program player if the same IMA ad
      // decision endpoints are blocked. Keep this exception site-scoped.
      requestDomains:['g.doubleclick.net','pagead2.googlesyndication.com'],
      topDomains:['wetv.vip'],resourceTypes:RESOURCE_TYPES.filter(type=>type!=='main_frame')
    }}];
  }
  function policy(c){
  ...
  const api={RESOURCE_TYPES,SOURCES,DEFAULTS,host,domain,config,recovering,enabledFor,parse,compile,compatibility,policy};
