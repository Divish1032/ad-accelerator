...
function notice(text,target='notice',kind='info'){const el=$(target);el.textContent=text;el.hidden=!text;el.dataset.kind=kind;el.setAttribute('role',kind==='error'?'alert':'status');}
...
async function refresh(){
 ...
 try{
  ...
  selection();
 }catch(e){if(id!==epoch||busy)return;lock(true);notice(e.message+' Reopen Privacy to reconnect.','notice','error');}
}
async function action(fn,success,target='notice'){lock(true);notice('',target);try{await fn();if(success)notice(success,target,'success');}catch(e){notice(e.message,target,'error');}finally{lock(false);await refresh();}}
...
$('cookies').addEventListener('change',()=>{
 ...
 void action(async()=>{if(!await grant)throw Error('Permission was declined. No cookie setting was changed.');await request('privacy-cookies',{blocked});},'Saved. Chrome’s current cookie status is shown below.','cookies-message');
});
