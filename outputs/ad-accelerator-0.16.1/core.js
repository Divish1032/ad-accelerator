  ...
  class Accelerator {
    ...
    update({ video, ad, enabled, now, speed = 10, live = false, adInfo = null }) {
      ...
      if(this.blocked&&ad&&enabled&&video&&!video.paused&&adInfo&&this.blockedEvidence&&this.retries<3){
        ...
        if(!this.rearm&&(newOrdinal||newCountdown||newMedia))this.rearm={video,info:adInfo,time:video.currentTime,now};
        const r=this.rearm;
        const progressing=r&&(r.info.kind==='separate-media'?
        ...
        if(r&&r.video===video&&progressing&&video.currentTime>r.time&&now-r.now>=250){this.blocked=false;this.rearm=null;this.candidate=null;this.retries++;}
      }
