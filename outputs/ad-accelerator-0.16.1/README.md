# Ad Accelerator 0.16.1 — Chrome Web Store release candidate

Version 0.16.1 fixes consecutive Prime Video ad pods: when generic detection recognizes the shared player first, the runtime now retains Prime's trusted ad number/countdown evidence so a player speed reset between ads can safely rearm acceleration for the next ad. Named on-demand integrations are Prime Video, JioHotstar, YouTube, Plex, Amazon MX Player, ZEE5, SonyLIV and WeTV. The generic HTML5 transition detector remains broader experimental coverage, not a universal compatibility claim.
...
259 automated tests pass. Prior user confirmation: Prime 10× and Hotstar acceleration worked; Prime, Hotstar, YouTube, Plex and Amazon MX Player playback were reported working in the previously installed build. The 0.16.1 Prime consecutive-ad repair, ZEE5, SonyLIV and WeTV protected ad workflows, and generic-first behavior remain manual native gates. Plex/WeTV fresh-load compatibility, native filter/privacy/upgrade checks, website-wide blocking and live-service compatibility also remain unverified. No claim is made that every embedded host is covered.
